...
class ComputerScienceSubjects extends StatelessWidget {
  const ComputerScienceSubjects({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        // Filtering subjects that belong to the CSE branch
        final mechanicalSubjects = controller.subjects
            .where((subject) => subject.branchId == 1) // Assuming 3 is the ID for CSE
            .toList();

        if (mechanicalSubjects.isEmpty) {
          return const Center(child: Text("No subjects available for CSE."));
        }
