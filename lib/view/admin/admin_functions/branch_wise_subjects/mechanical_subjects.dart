...
class MechanicalSubjects extends StatelessWidget {
  const MechanicalSubjects({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        // Filtering subjects that belong to the Mechanical branch
        final mechanicalSubjects = controller.subjects
            .where((subject) => subject.branchId == 3) // Assuming 3 is the ID for Mechanical
            .toList();

        if (mechanicalSubjects.isEmpty) {
          return const Center(child: Text("No subjects available for Mechanical."));
        }
